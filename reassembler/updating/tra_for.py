# Functions linked to the forward translocation (updating and correction).

import numpy as np
from Bio import Seq
from typing import Iterable

def fragment_tra_for(coord_del_1 : int, coord_del_2 : int, coord_ins : int, shape_matrix : int):
    """
...
start: int, end: int, start_paste: int, matrix: "np.ndarray[N,N]"
) -> "np.ndarray[N,N]":
    """
    ...
    fragment_to_modify_3 = np.concatenate(
            (
                matrix[start_paste + 1 : start, start : end + 1],
                matrix[start + size_insertion :, start : end + 1],
            ),
            axis=0,
    ...
    insertion_1 = np.concatenate(
            (
                ...
                np.zeros(
                    (
                        matrix.shape[0] - fragment_to_modify_1.shape[0],
                        size_insertion,
                    )
                ),
            ),
    ...
    insertion_2 = np.concatenate(
            (
                ...
                np.zeros(
                    (
                        matrix.shape[0] - (start_paste + 1 + size_insertion),
                        size_insertion,
                    )
                ),
            )
    ...
    insertion = np.concatenate(
            (
                np.zeros((matrix.shape[0], start_paste)),
                insertion_1 + insertion_2 + insertion_3,
                np.zeros(
                    (
                        matrix.shape[0],
                        matrix.shape[1] - start_paste - size_insertion,
                    )
                ),
            ),
            axis=1,
    ...
    # Insersion in the white spaces
    matrix = matrix + insertion + insertion_transposed

    # Correction of traces
    if (start_paste < matrix.shape[0]):  #  Sometimes, the coord is exactly scrambled.shape so there is nothing to delete after.
